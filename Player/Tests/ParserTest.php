...
EOF
        ...
        /** @var Scenario $scenario */
        $scenario = $scenarioSet->getIterator()[0];
        $block = $scenario->getBlockStep();

        $first = $block->getBlockStep();
        $this->assertInstanceOf(VisitStep::class, $first);
        $this->assertEquals("url('/')", $first->getUri());

        $second = $block->getNext();
        $this->assertInstanceOf(VisitStep::class, $second);
        $this->assertEquals("url('/doc')", $second->getUri());
    }

    /**
     * @expectedException \Blackfire\Player\Exception\LogicException
     * @expectedExceptionMessage A "group" must contains at least 1 step at line 1.
     */
    public function testEmptyGroupThrowsException()
    {
        $parser = new Parser();
        $parser->parse(<<<'EOF'
group empty_group
