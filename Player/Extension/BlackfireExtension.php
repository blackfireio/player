    ...
    private function continueSampling(ResponseInterface $response, Context $context, $checkProgress = true)
    {
        ...
        if (!$continue) {
            ...
            if ($progress < $prevProgress) {
                throw new LogicException('Profiling progress is inconsistent (progress is going backward). That happens for instance when the project\'s infrastructure is behind a load balancer. Please read https://blackfire.io/docs/cookbooks/reverse-proxies#configuration-load-balancer');
            }

            if ($progress === $prevProgress) {
                throw new LogicException('Profiling progress is inconsistent (progress is not increasing). That happens for instance when using a reverse proxy or an HTTP cache server such as Varnish. Please read https://blackfire.io/docs/cookbooks/reverse-proxies#reverse-proxies-and-cdns');
            }
